(function() {
    window.updateMenuCartCount = function() {
        fetch('ajax/get-cart-count.php')
            .then(response => response.json())
            .then(data => {
                if (data.success && typeof data.count !== 'undefined') {
                    document.querySelectorAll('.count.count-box:not(.wishlist-count)').forEach(function(el) {
    ...
    window.updateMenuWishlistCount = function() {
    ...
    var handleMobileMenu = function () {
        ...
        $desktopMenu.find("> li.menu-item").each(function (i, menuItem) {
            ...
            if (text.toLowerCase() === "home") {
                const homeLink = $item.find("> a.item-link").attr("href") || "index.php";
                $mobileMenu.append(
                    `<li class="nav-mb-item"><a href="${homeLink}" class="mb-menu-link"><span>${text}</span></a></li>`
                );
                return;
            }
  ...
  -------------------------------------------------------------------------*/
    ...
    var notifyForm = () => {
        $("#btnLogin").on("click", function (e) {
            ...
            // If valid, let the form submit normally to auth/login_handler.php
        });
    };
