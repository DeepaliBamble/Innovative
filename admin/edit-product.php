...
$imagesStmt = $pdo->prepare("SELECT * FROM product_images WHERE product_id = ? ORDER BY display_order");
$imagesStmt->execute([$id]);
$existingImages = $imagesStmt->fetchAll();

// Fetch existing product attributes
$attributesStmt = $pdo->prepare("SELECT * FROM product_attributes WHERE product_id = ? ORDER BY display_order");
$attributesStmt->execute([$id]);
$existingAttributes = $attributesStmt->fetchAll();

// Fetch existing product categories (multi-category)
$pcStmt = $pdo->prepare("SELECT category_id FROM product_categories WHERE product_id = ? ORDER BY is_primary DESC");
$pcStmt->execute([$id]);
$existingCategoryIds = array_column($pcStmt->fetchAll(), 'category_id');
// Fallback: if junction table empty, use primary category_id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $slugCheckStmt = $pdo->prepare("SELECT id FROM products WHERE slug = ? AND id != ?");
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $updateSql = "UPDATE products SET
                         name = ?, slug = ?, sku = ?, short_desc = ?, description = ?, shipping_returns = ?,
                         price = ?, sale_price = ?, cost_price = ?, category_id = ?,
                         image_path = ?, stock_quantity = ?, is_featured = ?, is_active = ?,
                         updated_at = NOW()
                         WHERE id = ?";
            ...
            // Update product_categories (multi-category)
            ...
            if (!empty($additional_images)) {
                ...
                foreach ($additional_images as $index => $img_path) {
                    ...
                    if (!empty($img_path)) {
                        ...
                        $imageStmt->execute([$id, $img_path, $display_order]);
                    }
                }
            }
            ...
            if (!empty($attributes)) {
                ...
                foreach ($attributes as $index => $attr) {
                    $display_order = $index + 1;
                    $attrStmt->execute([$id, $attr['name'], $attr['value'], $display_order]);
                }
            }
...
<div class="container-fluid">
    <div class="card">
        <div class="card-header">
            <h1 class="mb-0">Edit Product: <?= htmlspecialchars($product['name']) ?></h1>
        </div>
        <div class="card-body">
            <?php if (!empty($errors)): ?>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="short_desc" name="short_desc" rows="2"
                                              placeholder="Brief product description (1-2 sentences)"><?= htmlspecialchars($_POST['short_desc'] ?? $product['short_desc']) ?></textarea>
                                </div>

                                <div class="mb-3">
                                    <label for="description" class="form-label">Full Description</label>
                                    <textarea class="form-control" id="description" name="description" rows="6"
                                              placeholder="Detailed product description"><?= htmlspecialchars($_POST['description'] ?? $product['description']) ?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="hidden" name="image_path" id="image_path" value="<?= htmlspecialchars($_POST['image_path'] ?? $product['image_path']) ?>">
                                    ...
                                    <div id="mainImagePreview" style="<?= empty($_POST['image_path'] ?? $product['image_path']) ? 'display: none;' : '' ?>">
                                        <img id="mainPreviewImg" src="<?= !empty($_POST['image_path'] ?? $product['image_path']) ? '../' . htmlspecialchars($_POST['image_path'] ?? $product['image_path']) : '' ?>"
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="hidden" name="additional_images" id="additional_images" value="">
                    ...
                    <div class="col-lg-4">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="price" name="price" required
                                           step="0.01" min="0" value="<?= htmlspecialchars($_POST['price'] ?? $product['price']) ?>"
                                           placeholder="0.00">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="sale_price" name="sale_price"
                                           step="0.01" min="0" value="<?= htmlspecialchars($_POST['sale_price'] ?? $product['sale_price']) ?>"
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="cost_price" name="cost_price"
                                           step="0.01" min="0" value="<?= htmlspecialchars($_POST['cost_price'] ?? $product['cost_price']) ?>"
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                ...
                                $selectedCategoryIds = isset($_POST['category_ids'])
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <?php
                                // Determine checkbox states - if form was submitted, use POST values, otherwise use product values
                                $is_featured_checked = $_SERVER['REQUEST_METHOD'] === 'POST' ? isset($_POST['is_featured']) : $product['is_featured'];
                                $is_active_checked = $_SERVER['REQUEST_METHOD'] === 'POST' ? isset($_POST['is_active']) : $product['is_active'];
                                ?>
...
additionalImagesList = <?= json_encode(array_column($existingImages, 'image_path')) ?>;
...
<?php if (!empty($existingAttributes)): ?>
<?php foreach ($existingAttributes as $attr): ?>
addAttributeRow('<?= addslashes($attr['attribute_name']) ?>', '<?= addslashes($attr['attribute_value']) ?>');
<?php endforeach; ?>
<?php endif; ?>
</script>
